...
class Modal {
  constructor(app) { this.scope = new Scope(); this.contentEl = new Element(app.document); this.containerEl = new Element(app.document); this.closed = 0; }
  open() {}
  close() { this.closed++; }
}
...
test('Escape dismisses editing first, then closes; explicit close still works while editing',()=>{
 const {modal,document}=setup();modal.scope.callback();assert.equal(document.activeElement,null);assert.equal(modal.closed,0);
 modal.scope.callback();assert.equal(modal.closed,1);
 const other=setup();other.modal.close();assert.equal(other.modal.closed,1);assert.equal(other.modal.containerEl.listeners.size,0);
});
test('one backdrop gesture cannot blur and close the food logger',()=>{
 ...
 const oldNow=Date.now;Date.now=()=>oldNow()+1000;try { const next=event();modal.containerEl.listeners.get('click')(next);assert.equal(next.stopped,undefined); } finally {Date.now=oldNow;}
});
test('ordinary controls are not swallowed and cleanup removes the keyboard guard',()=>{
 ...
 modal.containerEl.listeners.get('click')(event);modal.close();assert.equal(modal.contentEl.listeners.size,0);assert.equal(modal.containerEl.listeners.size,0);
});
