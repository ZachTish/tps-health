...
const result = await build({stdin:{contents:['nutrients','format','native-daily-dashboard','food-data-quality','settings-normalization','health-property-catalog','types'].map(name=>`export * from './${name}';`).join('\n'),resolveDir:fileURLToPath(new URL('../src',import.meta.url)),loader:'ts'},bundle:true,format:'esm',platform:'node',write:false});
