...
## September 18 follow-up — White Claw search (0.47.2)

The initial audit did not catch a critical distinction between Open Food Facts' discovery index and current product records. Live inspection on September 18 found that the first 40 “white claw” index hits omitted serving metadata, and some indexed nutrient/name/brand values differed from the current product. The prior implementation converted those summaries directly into 100 g foods and fetched the full product only after selection. That was insufficient for trustworthy search results.

The replacement uses one [documented bulk product lookup](https://openfoodfacts.github.io/openfoodfacts-server/api/ref-cheatsheet/#get-data-for-a-list-of-products) for matching discovered codes before normalization/display. Returned codes must match requested identities; missing and nutrient-empty full records cannot reuse stale index nutrition. Full-serving nutrition, serving units and provenance stay together. A shared rate-limit cooldown and existing query/product caches bound traffic; no individual request for each result. Exact-product selection reuses these records. More complete labeled matches rank first among equally relevant unused provider results, after saved notes.

The captured record `0635985500018` supplies 355 ml, 100 kcal and 2 g carbohydrate; `0635985802761` explicitly supplies one 355 ml can of Zero Proof, 15 kcal and 5 g carbohydrate; `0635985801986` supplies a 568 ml can and 170 kcal. These are separate products. The manufacturer [Black Cherry page](https://www.whiteclaw.com/products/black-cherry) and [12 oz variety pack](https://shop.whiteclaw.com/products/white-claw_seltzer_flavor_no_2_variety_12pk_12oz_can_5_0_abv) corroborate the original 12 oz product's 100 calories, 2 g carbohydrate and 5% ABV. OFF's Black Cherry record has a conflicting ABV, and other records describe beverages in grams. Fetching complete data does not repair those upstream errors. No guessed 355 ml serving, density, ABV or brand-wide nutrient override is applied. The source review indicator remains; saved label-checked notes take priority.

The current details disclosure supersedes the 0.47.0 explanatory presentation described above: only the base serving and nutrient amounts are shown, all nine core fields plus reported extras, with unknowns distinguished from measured zero. Receipt data and quality assessment remain available internally/in saved imports. Public test fixtures include source/date/license attribution. Verification and artifact hashes are recorded in `RELEASE-0.47.2.md`.
